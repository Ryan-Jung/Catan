module.exports = (db) => {
  ...
  playerFunctions.findPlayerGames = (userId) => {
    return db.any('SELECT id,game_name,player_limit,p.player_count FROM "games" g '
                  ...
                  +              'FROM "players" WHERE user_id = $1 GROUP BY game_id) p '
                  +'ON p.game_id = g.id '
                  ,[userId]);
  }

  playerFunctions.findPlayerId = (userId,gameId) => {
    return db.one('SELECT id FROM "players" WHERE user_id = $1 AND game_id = $2',
          [userId,gameId]);
  }

  playerFunctions.addDevCard = (userId, gameId, devCard) => {
    return playerFunctions.findPlayerId(userId,gameId)
    .then( ({id}) =>
